...
## 生成工作流 + 命名规则（硬约束）
1. **preview + verify 确认**：`shortcuts-toolkit preview --verify -i spec.json`（macOS 用系统真实 identifier 复核），把「操作+模块+警告」呈现给用户确认。第三方 App 动作 / 未知内置动作 / reference 错误 token 会导致导入后「无法找到此操作」，必须提前发现（用 `verify` 最权威）。
